-- Expose enabled tenant payment gateways to portal clients (avoids RLS/anon access issues)
...
BEGIN
  ...
  SELECT
    ...
    tpg.instructions,
    tpg.sort_order,
    COALESCE(tpg.bkash_mode, 'tokenized')
  ...
  IF NOT FOUND THEN
    ...
    SELECT
      ...
      pgs.instructions,
      pgs.sort_order,
      COALESCE(pgs.bkash_mode, 'tokenized')
...
-- Tighten execution: anyone can call but only for reading enabled methods
