-- Fix failed migration: must DROP before changing OUT param types
DROP FUNCTION IF EXISTS public.get_customer_profile(uuid);

-- Recreate with correct types (customers date columns are DATE)
...
-- Also ensure ticket RPC bypasses row security (fixes "Customer not found" under RLS)
