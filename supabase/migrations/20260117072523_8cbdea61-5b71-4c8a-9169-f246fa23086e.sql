-- Fix wallet deduction RPC ambiguity by removing overloaded functions
-- and re-creating a single canonical signature.

DROP FUNCTION IF EXISTS public.use_wallet_for_recharge(uuid, numeric, uuid, text);
DROP FUNCTION IF EXISTS public.use_wallet_for_recharge(uuid, numeric, text, uuid);
