...
const getTemplateConfig = (templateId: string) => {
  ...
  if (enhanced) {
    return {
      ...
      heroLayout: enhanced.heroLayout,
...
export default function TenantLanding() {
  ...
  return (
    <div className="min-h-screen">
      {/* Header - ISP Point Style */}
      <header className={`${template.headerClass} sticky top-0 z-50 shadow-md`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            {/* Logo with Home Badge */}
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                {tenant.logo_url ? (
                  ...
                  <div className={`h-10 lg:h-12 w-10 lg:w-12 rounded-xl bg-gradient-to-br ${themeColors.gradient} flex items-center justify-center shadow-lg ${themeColors.glow}`}>
                    <Wifi className="h-6 w-6 text-white" />
                  </div>
                )}
              </div>
              {/* Home badge next to logo */}
              <button
                ...
                <div key={item.id} className="relative group">
                  {item.url && !item.subItems?.length ? (
                    <a
                      ...
                      className={`px-4 py-2 text-sm font-medium transition-all rounded-lg flex items-center gap-1.5 ${
                        template.isDark 
                    ...
                    <>
                      ...
                      >
                        {item.label}
                        <ChevronRight className={`h-3.5 w-3.5 transition-transform ${openDropdown === item.id ? 'rotate-90' : ''}`} />
                      </button>
                      {/* Enhanced Dropdown */}
                      {openDropdown === item.id && (
                        <div 
                        ...
                        >
                          {item.subItems.map((sub, idx) => (
                            ...
                            >
                              <div className={`h-8 w-8 rounded-lg bg-gradient-to-br ${themeColors.gradient} flex items-center justify-center`}>
            ...
            <div className="hidden md:flex items-center gap-2 lg:gap-3">
              ...
              {tenant.landing_page_contact_phone && (
                <a 
                  ...
                  className={`hidden xl:flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-all ${
                    template.isDark 
                      ? 'text-white/90 hover:bg-white/10' 
                      : 'text-gray-700 hover:bg-gray-100'
                  }`}
                >
                  <div className={`h-7 w-7 rounded-full bg-gradient-to-br ${themeColors.gradient} flex items-center justify-center`}>
            ...
            <button 
              className={`lg:hidden p-2.5 rounded-xl transition-all ${
                template.isDark ? 'hover:bg-white/10' : 'hover:bg-gray-100'
        ...
        {mobileMenuOpen && (
          <div className={`lg:hidden ${template.isDark ? 'bg-gray-900/95 backdrop-blur-xl border-t border-white/10' : 'bg-white/95 backdrop-blur-xl border-t shadow-xl'} px-4 py-6`}>
